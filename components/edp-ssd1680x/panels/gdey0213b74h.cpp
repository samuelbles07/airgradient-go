...
GDEY0213B74H::GDEY0213B74H(const ssd1680x::Config& cfg) : ssd1680x::Device(cfg) {}

esp_err_t GDEY0213B74H::_set_ram_addr_defaults() {
  if (_mode == Mode::Full180) {
    return _set_ram_addr_defaults_180();
  }
  return _set_ram_addr_defaults_full();
}

esp_err_t GDEY0213B74H::_set_ram_addr_defaults_full() {
...
esp_err_t GDEY0213B74H::_set_ram_addr_defaults_180() {
...
esp_err_t GDEY0213B74H::_trigger_update_full_tx() {
  EDP_RETURN_ON_ERROR(write_cmd(0x22));
  EDP_RETURN_ON_ERROR(write_data_u8(0xF7));
  return write_cmd(0x20);
}

esp_err_t GDEY0213B74H::_trigger_update_fast_tx() {
  EDP_RETURN_ON_ERROR(write_cmd(0x22));
  EDP_RETURN_ON_ERROR(write_data_u8(0xC7));
  return write_cmd(0x20);
}

esp_err_t GDEY0213B74H::_trigger_update_partial_tx() {
  EDP_RETURN_ON_ERROR(write_cmd(0x22));
  EDP_RETURN_ON_ERROR(write_data_u8(0xFF));
  return write_cmd(0x20);
}

esp_err_t GDEY0213B74H::_write_ram(uint8_t ram_cmd, const uint8_t* buf, size_t len) {
...
esp_err_t GDEY0213B74H::_set_window(uint8_t x_start_bytes, uint8_t x_end_bytes, uint16_t y_start,
                                    uint16_t y_end) {
...
esp_err_t GDEY0213B74H::_set_cursor(uint8_t x_bytes, uint16_t y) {
  EDP_RETURN_ON_ERROR(write_cmd(0x4E));
...
esp_err_t GDEY0213B74H::init_full() {
...
esp_err_t GDEY0213B74H::init_full_180() {
  esp_err_t err = ESP_OK;
  EDP_RETURN_ON_ERROR(init());
  ...
  EDP_GOTO_ON_ERROR(write_cmd(0x11), out_cfg);
  EDP_GOTO_ON_ERROR(write_data_u8(0x02), out_cfg);
  ...
  EDP_GOTO_ON_ERROR(_set_ram_addr_defaults_180(), out_cfg);
...
esp_err_t GDEY0213B74H::ensure_init_full() {
  if (!_asleep && (_mode == Mode::Full || _mode == Mode::Full180)) {
    return ESP_OK;
  }
  return init_full();
}

esp_err_t GDEY0213B74H::ensure_init_full_180() {
  if (!_asleep && _mode == Mode::Full180) {
    return ESP_OK;
  }
  return init_full_180();
}

esp_err_t GDEY0213B74H::init_fast() {
  esp_err_t err = ESP_OK;
  EDP_RETURN_ON_ERROR(init());
...
esp_err_t GDEY0213B74H::ensure_init_fast() {
...
esp_err_t GDEY0213B74H::display_frame_bw(const uint8_t* buf, size_t len) {
  ...
  esp_err_t err = ESP_OK;
  EDP_RETURN_ON_ERROR(bus_acquire());
  EDP_GOTO_ON_ERROR(_set_ram_addr_defaults(), out);
...
esp_err_t GDEY0213B74H::display_frame_bw_fast(const uint8_t* buf, size_t len) {
  if (buf == nullptr || len != BUFFER_SIZE) {
    return ESP_ERR_INVALID_ARG;
  }
...
esp_err_t GDEY0213B74H::clear_white() {
...
esp_err_t GDEY0213B74H::clear_black() {
...
esp_err_t GDEY0213B74H::set_basemap_bw(const uint8_t* buf, size_t len) {
  if (buf == nullptr || len != BUFFER_SIZE) {
    return ESP_ERR_INVALID_ARG;
  }
...
esp_err_t GDEY0213B74H::display_partial_bw(int x, int y, int w, int h, const uint8_t* buf,
                                          size_t len) {
...
esp_err_t GDEY0213B74H::partial_begin() {
  if (_mode == Mode::Full180) {
  ...
  // Partial flow expects the controller to be in a non-rotated configuration.
  // If we are in some other mode, reset to full mode first.
  if (_asleep) {
...
esp_err_t GDEY0213B74H::partial_write_bw(int x, int y, int w, int h, const uint8_t* buf, size_t len) {
  ...
  const uint16_t y_start = (uint16_t)y;
  const uint16_t y_end = (uint16_t)(y + h - 1);

  esp_err_t err = ESP_OK;
  EDP_RETURN_ON_ERROR(bus_acquire());
  EDP_GOTO_ON_ERROR(_set_window(x_start_bytes, x_end_bytes, y_start, y_end), out);
  EDP_GOTO_ON_ERROR(_set_cursor(x_start_bytes, y_start), out);
  EDP_GOTO_ON_ERROR(_write_ram(0x24, buf, len), out);
...
esp_err_t GDEY0213B74H::partial_end_update() {
  if (!_partial_prepared) {
    return ESP_ERR_INVALID_STATE;
  }
  EDP_RETURN_ON_ERROR(init());
...
esp_err_t GDEY0213B74H::partial_update() {
  return partial_end_update();
}

esp_err_t GDEY0213B74H::display_partial_bw_all(const uint8_t* buf, size_t len) {
  if (buf == nullptr || len != BUFFER_SIZE) {
    return ESP_ERR_INVALID_ARG;
  }
...
esp_err_t GDEY0213B74H::deep_sleep() {
  EDP_RETURN_ON_ERROR(init());
